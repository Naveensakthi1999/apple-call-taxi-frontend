...
function OwnerReport() {
  ...
  const fetchlength = () => {
    axios.get(`${API_BASE_URL}/report/owner_report_count?search=${search}&date=${date}`)
      .then(result => {
  ...
  const fetchData = () => {
    axios.get(`${API_BASE_URL}/report/owner_report_list?page=${page}&limit=${rowsPerPage}&search=${search}&date=${date}`)
      .then((result) => {
        if (result.data.Status) {
  ...
  const handlePrint = async (id, name) => {
    try {
      const response = await axios.get(`${API_BASE_URL}/report/export_owner_report?id=${id}&date=${date}`, {
        responseType: 'arraybuffer',
      });

      const blob = new Blob([response.data], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
      saveAs(blob, `${name}-${dayjs(date).format('MMMM YYYY')}-monthly_status.xlsx`);
    } catch (error) {
                  ...
                  return (
                    <tr key={item.id}>
                      ...
                      <td><Button
                              ...
                              type="button"
                              color="secondary"
                              onClick={() => handlePrint(item.id, item.firstname)}
                          >
                              Export
                          </Button>
                      </td>
