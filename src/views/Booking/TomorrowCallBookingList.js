...
function TomorrowCallBooking() {
  ...
  const fetchlength = () => {
    axios.get(`${API_BASE_URL}/booking/tomorrow_booking_count?search=${search}`)
    .then(result => {
      if(result.data.Status) {
  ...
  const fetchData = () => {
    axios.get(`${API_BASE_URL}/booking/tomorrow_booking_list?page=${page}&limit=${rowsPerPage}&search=${search}`)
      .then((result) => {
        if (result.data.Status) {
        ...
        <table className="table table-hover">
          ...
          {items.map((item, i) => (
            <tr key={item.id}>
              ...
              <td>{item.car_name}</td>
              <td>{item.payment_status}<br></br>{ item.amount != '' ? 'Rs.'+ item.amount : '' }</td>
         ...
         : <tbody><td className='text-center' colSpan={16}>No matching records found</td></tbody>}
         </table>
      </div>
...
export default TomorrowCallBooking;   
