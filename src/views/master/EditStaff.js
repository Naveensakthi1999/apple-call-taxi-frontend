...
useEffect(() => {
  const fetchData = async () => {
    try {
      const result = await axios.get(`${API_BASE_URL}/staff/edit_staff/${id}`);
      const staffData = result.data.Result[0];
      setValues((prevValues) => ({
        ...prevValues,
        date: new Date(staffData.date).toLocaleDateString(),
        ...
        birth_date: new Date(staffData.birth_date).toLocaleDateString(),
  ...
  const handleSubmit = (e) => {
    ...
    axios.put(`${API_BASE_URL}/staff/update_staff/`+id, formData)
    .then(result => {
