...
function ManageOwner() {
  ...
  const fetchlength = () => {
    axios.get(`${API_BASE_URL}/manageowner/manageowner_count?search=${search}`)
    .then(result => {
      if(result.data.Status) {
  ...
  const fetchData = () => {
    axios.get(`${API_BASE_URL}/manageowner/manageowner_list?page=${page}&limit=${rowsPerPage}&search=${search}`)
      .then((result) => {
        if (result.data.Status) {
  ...
  const handleDelete = (id) => {
    ...
    .then((result) => {
      if (result.isConfirmed) {
        ...
        axios.put(`${API_BASE_URL}/manageowner/delete_manageowner/`+id, {deleteBy})
        .then(result => {
            if(result.data.Status) {
              fetchData();
              fetchlength();
  ...
  const handleInActive = (id) => {
    ...
    .then((result) => {
      if (result.isConfirmed) {
        ...
        axios.put(`${API_BASE_URL}/manageowner/status_manageowner/`+id, {status})
        .then(result => {
            if(result.data.Status) {
              fetchData();
            } else {
        ...
        <table className="table table-hover">
          ...
          {items.map((item, i) => (
            <tr key={item.id}>
              ...
              <td>
                <Link to={`/master/EditManageOwner/` + item.id}>
                <Tooltip title="Edit">
