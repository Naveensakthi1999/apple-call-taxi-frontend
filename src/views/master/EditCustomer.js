...
function EditCustomers() {
    ...
    useEffect(() => {
        const fetchData = async () => {
          try {
            const result = await axios.get(`${API_BASE_URL}/customer/edit_customer/${id}`);
            const customerData = result.data.Result[0];
            setValues((prevValues) => ({
              ...prevValues,
                name: customerData.name,
                mobile: customerData.mobile,
                adl_mobile: customerData.adl_mobile,
            }));
    ...
    const handleSubmit = (e) => {
        ...
        if (Object.keys(formErrors).length > 0) {
          ...
          return;
            ...
            axios.put(`${API_BASE_URL}/customer/update_customer/`+id, { values })
                .then(result => {
