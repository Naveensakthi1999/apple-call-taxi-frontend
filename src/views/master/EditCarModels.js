...
function EditCarModels() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await axios.get(`${API_BASE_URL}/carmodels/edit_carmodels/${id}`);
        const carData = result.data.Result[0];
        setValues((prevValues) => ({
          ...prevValues,
          carname: carData.carname,
          available_seats: carData.available_seats,
          min_trip_amount: carData.min_trip_amount,
          min_service_charge: carData.min_service_charge,
          max_service_charge: carData.max_service_charge,
          fixed_amount: carData.fixed_amount,
          per_kms: carData.per_kms
        }));
  ...
  const handleSubmit = (e) => {
    ...
    axios.put(`${API_BASE_URL}/carmodels/update_carmodels/` + id, { values })
      .then(result => {
