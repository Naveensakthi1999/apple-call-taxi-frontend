...
function Loaction() {
  ...
  const fetchlength = () => {
    axios.get(`${API_BASE_URL}/location/location_count?search=${search}`)
      .then(result => {
        if (result.data.Status) {
  ...
  const fetchData = () => {
    axios.get(`${API_BASE_URL}/location/location_list?page=${page}&limit=${rowsPerPage}&search=${search}`)
      .then((result) => {
        if (result.data.Status) {
    ...
    withReactContent(Swal)
      ...
      .then((result) => {
        if (result.isConfirmed) {
          ...
          axios.put(`${API_BASE_URL}/location/delete_location/` + id, { deleteBy })
            .then(result => {
              if (result.data.Status) {
                fetchData();
                fetchlength();
      ...
      <div className=" example">
        <table className="table table-hover">
          ...
          {length > 0 ?
            <tbody>
              {items.map((item, i) => (
                <tr key={item.id}>
                  ...
                  <td>{item.name}</td>
                  <td>
                    <Link to={`/master/EditLocation/` + item.id}>
                      <Tooltip title="Edit">
            ...
            : <tbody><td className='text-center' colSpan={5}>No matching records found</td></tbody>}
        </table>
      </div>
